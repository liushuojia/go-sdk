...
const TableNameOrderArchive = "orderArchive"

// OrderArchive mapped from table <orderArchive>
type OrderArchive struct {
	...
	Status                       int64          `gorm:"column:status;not null;comment:0 保存中	1 有效 -1 审核流程中" json:"status"`                                    // 0 保存中	1 有效 -1 审核流程中
	...
	InvoiceReceivedFlag          int64          `gorm:"column:InvoiceReceivedFlag;not null;comment:是否回收完发票" json:"InvoiceReceivedFlag"`                      // 是否回收完发票
}

func (m *OrderArchive) IsEmpty() bool {
	if m == nil {
		return true
	}
	return m.ID == 0
...
// TableName OrderArchive's table name
func (*OrderArchive) TableName() string {
	return TableNameOrderArchive
}
